...
local relevance = require"relevance_metric"
...
local filter={
		is= {
			wikidata= function(url)
			 ...
			 dbpedia = function(url,is_property, ntype) --ntype inverse
...
local function get_data_description(url,code)
     ...
     if not description and code then
...
local function execute_immediate_class(entity_property, description, url,class_instance)
		local str = nil
...
local function get_immediate_class(url,code)
	...
	if not str then
        ...
        if not  str then
			...
			str = execute_immediate_class(entity_property, description, url, class_instance)
	        end
	        if str then
        	        return str
	        end
...
local function similairy_distance (url_source,url_target ,code_source, code_target)
	relevance.init(client6379, client6380)
	local value = relevance.similarity_entity( url_source,url_target ,code_source, code_target, get_data_description)
	if value and code_source and code_target then
		client6379:zadd("list_adj_entity:"..sequence..":"..code_source,value,code_target)
	end
end

local factor_similarity = 0

local function get_adjacency_list(start)
	factor_similarity =  0

	local porcentagem  =  0.5

	local ncount = client6379:zcount("list_adj_entity:"..sequence..":"..start,"-inf","+inf")
	local size = ncount*porcentagem
	size = math.ceil(size)
	if size > ncount then
		size= ncount
	end
	if size > 0 then
        ...
        local list = client6379:zrevrange("list_adj_entity:"..sequence..":"..start,size, size,"withscores")

        for _,values in pairs(list) do
                local val = tonumber(values[2])
		client6379:hset("teste_adja", val, "")
		if val >= 0 then
			factor_similarity = val
		else
			factor_similarity =  1000
		end
	end
end
local function compare_similarity(code_source ,code_target)
        if code_source == code_target then
                return 1
        end
        local key = code_source.."@"..code_target
        if code_source > code_target then
                key = code_target.."@"..code_source
        end

        if   client6379:hexists("teste_similarity",key) then
                local value = client6379:hget("teste_similarity", key)
...
callback ={
	...
	extend = {
			...
			local validate, code_prop = filter.is[database](property,true,values.ntype)
                        if validate and ic_start and #ic_start>0 then
                                for  _,single_val in pairs(values.value) do
                                        path = {}
                                        meta_path ={}
                                        table.insert(path, start) --get code of estart
					...
					single_val = url_web.unescape(single_val)
							...
							value = compare_similarity(start ,cod)
						else
							value = 1000
						end
						if value >= factor_similarity then
					...
					single_val = url_web.unescape(single_val)
						...
						if close then	
								...
								if client6379:hsetnx("h_wobject_immediate_class:"..sequence, start .."@"..cod,"")then 
								 	client6379:zadd("list_adj_entity:"..sequence..":"..start,0,cod)
        ...
        immediate_class = {
				...
				similairy_distance (url_source,url_target ,code_source, code_target)
